...
def notify_discord_on_fotobooking_change(sender, instance, created, **kwargs):
    """
    Send en Discord notifikation når en FotoBooking oprettes eller opdateres.
    ...
    try:
        ...
        # Byg booking URL
        booking_url = f"{settings.CSRF_TRUSTED_ORIGINS[0]}/Foto/FotoBooking/{instance.id}/"
...
""".strip()
        ...
        success = apobj.notify(
            body=message,
            title=f"📷 Foto - {instance.team.name}",
        ...
        if success:
            ...
            logger.warning(f'Fejl ved afsendelse af Discord notifikation for FotoBooking {instance.id}')
    
    except Exception as e:
        logger.error(
            f'Fejl ved afsendelse af Discord notifikation for FotoBooking {instance.id}: {str(e)}',
            exc_info=True
        )
